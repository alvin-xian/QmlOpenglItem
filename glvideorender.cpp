...
GlVideoRenderer::GlVideoRenderer(int width, int height)
    :m_program(nullptr)
    ,m_zOrder(0)
    ,m_left(1)
    ,m_top(0)
    ,m_right(0)
    ,m_bottom(1)
    ,m_renderMode(IgnoreAspectRatio)
    ,m_textureWidth(-1)
    ,m_textureHeight(-1)
    ,m_targetWidth(width)
...
int GlVideoRenderer::setStreamProperties(int zOrder, float left, float top, float right, float bottom)
{
    m_zOrder = zOrder;
    m_left = left;
    m_top = top;
...
int GlVideoRenderer::getStreamProperties(int& zOrder, float& left, float& top, float& right, float& bottom)
{
    zOrder = m_zOrder;
    left = m_left;
    top = m_top;
...
void GlVideoRenderer::setFrameInfo(int rotation, bool mirrored)
{
...
inline QOpenGLFunctions* GlVideoRenderer::renderer()
{
    return QOpenGLContext::currentContext()->functions();
}

QOpenGLShaderProgram* GlVideoRenderer::createProgram()
{
...
#include <QDebug>
int GlVideoRenderer::initialize(int width, int height)
{
    std::lock_guard<std::mutex> lock(m_mutex);
    if (m_program)
        return 0;
...
int GlVideoRenderer::applyVertices()
{
    std::lock_guard<std::mutex> lock(m_mutex);
    if (!m_program)
        return -1;
...
int GlVideoRenderer::renderFrame(const YuvFrame& videoFrame)
{
    ...
    if (r)
        return r;
    QOpenGLFunctions *f = renderer();
//    f->glClear(GL_COLOR_BUFFER_BIT);//为了背景透明没有清除GL_COLOR_BUFFER_BIT颜色缓冲区，如果出现问题，将这个打开
...
void GlVideoRenderer::initializeTexture(int name, int id, int width, int height)
{
    QOpenGLFunctions *f = renderer();
...
void GlVideoRenderer::setupTextures(const YuvFrame& frameToRender)
{
...
void GlVideoRenderer::updateTextures(const YuvFrame& frameToRender)
{
    const GLsizei width = frameToRender.width;
    const GLsizei height = frameToRender.height;
    ...
    glTexSubImage2D(width, height, frameToRender.stride,
                    frameToRender.y());
    f->glActiveTexture(GL_TEXTURE1);
    f->glBindTexture(GL_TEXTURE_2D, m_textureIds[1]);
    glTexSubImage2D(width/2, height/2, frameToRender.stride/2,
                    frameToRender.u());
...
void GlVideoRenderer::glTexSubImage2D(GLsizei width, GLsizei height, int stride, const uint8_t* plane)
{
    QOpenGLFunctions *f = renderer();
    if (stride == width)
    {
...
int GlVideoRenderer::ajustVertices()
{
    if (m_left < -90) {
        return -1;
    }
...
int GlVideoRenderer::adjustCoordinates(int frWidth, int frHeight, int targetWidth, int targetHeight, int renderMode)
{
    ...
    int twidth = frWidth;//图片的宽
    int theight = frHeight;//图片的高
    float orig_ratio = (frHeight + 0.0f) / frWidth;//图片高除以宽。
    float require_ratio = (targetHeight + 0.0f) / targetWidth;//窗口高/宽
    ...
    if (renderMode == KeepAspectRatioByExpanding) {
        if (require_ratio <= orig_ratio) { //窗口较宽
            theight = require_ratio * frWidth;//目标图片的高度压缩
            ...
            twidth = frHeight / require_ratio;  //目标图片的宽度压缩

            left = 0 - (frWidth / (twidth + 0.0f) - 1.0f) / 2;
            right = 1 + (frWidth / (twidth + 0.0f) - 1.0f) / 2;
        }
    } else if (renderMode == KeepAspectRatio) {
    ...
    }else if (renderMode == IgnoreAspectRatio) {
        left = 0 ;
        right = 1 ;
        bottom = m_bottom;
        top = m_top;
